...
public class MailKitEmailSender : IApplicationEmailSender
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(toEmail))
            throw new ArgumentException("Champ manquant : veuillez indiquer un destinataire", nameof(toEmail));
        ...
        {
            HtmlBody = htmlBody,
            TextBody = "Impossible d’afficher ce message : votre client ne gère pas le HTML."
        };
        ...
        _logger.LogInformation(
            "Courriel envoyé à {Email} avec le sujet {Subject}.",
             toEmail,
             subject);
    }
}